...
function setTris(S)
	...
	local TB=0x00
	if(_btst(S,0x10))then TB = _bor(TB,0x02) else TB = _bor(TB,0x01) end
	if(_btst(S,0x20))then TB = _bor(TB,0x08) else TB = _bor(TB,0x04) end
	if(_btst(S,0x40))then TB = _bor(TB,0x20) else TB = _bor(TB,0x10) end
	if(_btst(S,0x80))then TB = _bor(TB,0x80) else TB = _bor(TB,0x40) end

	regWrite(0x00,T)
	regWrite(0x07,TB)

	--電流吸い込み対策。入力に設定したピンをHIGHにする
    if(not _btst(S,0x01))then PIO_DATA = _bor(PIO_DATA, 0x01) end
    if(not _btst(S,0x02))then PIO_DATA = _bor(PIO_DATA, 0x02) end
    if(not _btst(S,0x04))then PIO_DATA = _bor(PIO_DATA, 0x04) end
    if(not _btst(S,0x08))then PIO_DATA = _bor(PIO_DATA, 0x08) end
    if(not _btst(S,0x10))then PIO_DATA = _bor(PIO_DATA, 0x10) end
    if(not _btst(S,0x20))then PIO_DATA = _bor(PIO_DATA, 0x20) end
    if(not _btst(S,0x40))then PIO_DATA = _bor(PIO_DATA, 0x40) end
    if(not _btst(S,0x80))then PIO_DATA = _bor(PIO_DATA, 0x80) end
